...
const getSimilarEvents = async (req, res) => {
  try {
    ...
    const keywords = (event.title || '').split(' ').filter(w => w.length > 2).slice(0, 5);
    ...
    const similar = await Event.find({
      ...
      $or: [
        ...
        { type: event.type },
        ...(keywords.length ? [{ title: { $regex: keywords.join('|'), $options: 'i' } }] : []),
      ],
    })
      .populate('organizer', 'fullName')
      .select('title description startDate endDate location category images organizer capacity participants type')
      .limit(6)
      .sort({ startDate: 1 })
      .lean();

    // Attach real booked-ticket counts
    const ids = similar.map(e => e._id);
    let bookedMap = {};
    if (ids.length) {
      const agg = await Reservation.aggregate([
        { $match: { event: { $in: ids }, status: { $ne: 'cancelled' } } },
        { $group: { _id: '$event', total: { $sum: '$numberOfTickets' } } },
      ]);
      bookedMap = Object.fromEntries(agg.map(b => [b._id.toString(), b.total]));
    }

    const now = new Date();
    const enriched = similar.map(ev => {
      const booked = bookedMap[ev._id.toString()] || 0;
      return {
        ...ev,
        _bookedTickets: booked,
        participants:   [],
        participantCount: Array.isArray(ev.participants) ? ev.participants.length : 0,
        availableSpots: Math.max(0, (ev.capacity || 0) - booked),
        isFull:         Math.max(0, (ev.capacity || 0) - booked) <= 0,
      };
    });
